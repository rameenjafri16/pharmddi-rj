#!/bin/bash
#SBATCH --job-name=ablation_no_fix4
...
#SBATCH --mem=256G
#SBATCH --output=/scratch/rjafri/rj_subset_exp1/outputs/logs/ablation_no_fix4_%j.out
#SBATCH --error=/scratch/rjafri/rj_subset_exp1/outputs/logs/ablation_no_fix4_%j.err
...
python scripts/run_subset_pilot.py \
    --config configs/config_ablation_no_fix4.yaml \
    --n-pairs 4000 \
    --seed 42
...
if [ $EXIT_CODE -eq 0 ]; then
    echo ""
    echo "=== RESULTS ==="
    REPORT="/scratch/rjafri/rj_subset_exp1/outputs/ablation_no_fix4/comparison_report.txt"
    if [ -f "$REPORT" ]; then
        cat "$REPORT"
    fi
fi
