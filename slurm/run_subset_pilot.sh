#!/bin/bash
#SBATCH --job-name=rj_subset_pilot
...
#SBATCH --mem=256G
#SBATCH --output=/scratch/rjafri/rj_subset_exp1/outputs/logs/pilot_%j.out
#SBATCH --error=/scratch/rjafri/rj_subset_exp1/outputs/logs/pilot_%j.err
...
python scripts/run_subset_pilot.py \
    --config configs/config_pilot.yaml \
    --n-pairs 4000 \
    --seed 42
...
if [ $EXIT_CODE -eq 0 ]; then
    echo ""
    echo "=== RESULTS ==="
    REPORT="/scratch/rjafri/rj_subset_exp1/outputs/rj_subset_exp1/comparison_report.txt"
    if [ -f "$REPORT" ]; then
        cat "$REPORT"
    fi
fi
